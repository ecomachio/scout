...
export class PlayerOfTheMatchComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.passCount = this.match.passCount;
  }

  async onPlayerChoose(e: Player) {
    this.match.playerOfTheMatch = this.players.find(
      (p: Player) => p.id === e.id
    );
  ...
  async onPassCountConfirmed() {
    this.match.passCount = Number(this.passCount);
    await this.matchService.updateMatch(this.match, this.match.id);
    this.modalController.dismiss({
      passCount: this.match.passCount,
    });
  }
}
