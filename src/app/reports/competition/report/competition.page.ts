  ...
  async loadCompetition(competitionId) {
    ...
    this.competitionService
      ...
      .subscribe(async (res) => {
        ...
        this.goals = this.matches.reduce(
          (pv, cv, i) => pv + cv.score.away + cv.score.home,
          0
        );

        let playersByCatPromises: Array<
          Promise<QuerySnapshot<firebase.firestore.DocumentData>>
        > = [];

        this.categoriesQtd = new Set(
          this.matches.map((m) => m.category.id)
        ).size;

        playersByCatPromises = this.matches.map((m) =>
          this.playerService.getPlayersByCategory(m.category.id)
        );

        Promise.all(playersByCatPromises).then((playersQuerySnapshot) => {
          const allPLayers = [];

          playersQuerySnapshot.forEach(
            (pp: QuerySnapshot<firebase.firestore.DocumentData>) => {
          ...
          this.players = Array.from(new Set(allPLayers.map((a) => a.id))).map(
            (id) => allPLayers.find((a) => a.id === id)
          );
        });
      });
  }
